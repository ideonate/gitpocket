...
export function renderDetail(item) {
    ...
    content.innerHTML = `
        ...
        ${appState.comments.length === 0 ? '<div style="text-align: center; color: #999; padding: 32px; background: #f5f5f5; border-radius: 12px; margin: 16px 0;"><p style="margin: 0;">No comments</p></div>' : ''}
    `;
...
async function renderPRActions(pr) {
    ...
    try {
        ...
        // Add link to GitHub compare view
        const compareUrl = `https://github.com/${owner}/${repo}/pull/${pr.number}/files`;
        actionsHTML += `
            <div style="margin-bottom: 16px;">
                <a href="${compareUrl}" target="_blank" rel="noopener noreferrer"
                   style="display: inline-flex; align-items: center; gap: 8px; padding: 8px 16px; background: #24292e; color: white; text-decoration: none; border-radius: 6px; font-size: 14px; font-weight: 500;">
